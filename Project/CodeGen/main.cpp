...
int main()
{
	...
	wstring strCppPath = strProjPath + L"Script\\CScriptManager.cpp";
	wstring strHeaderPath = strProjPath + L"Script\\CScriptManager.h";
	...
	fwprintf_s(pFile, L"class CScriptManager\n{\n");
	fwprintf_s(pFile, L"public:\n\tstatic void GetScriptInfo(vector<wstring>& vec);\n");
	fwprintf_s(pFile, L"\tstatic CScript * GetScript(const wstring& strScriptName);\n");
	fwprintf_s(pFile, L"\tstatic CScript * GetScript(UINT iScriptType);\n");
	fwprintf_s(pFile, L"\tstatic const wchar_t * GetScriptName(CScript * pScript);\n};\n");
	...
	fwprintf_s(pFile, L"#include \"CScriptManager.h\"\n\n");
	...
	// 두번째 함수 작성
	fwprintf_s(pFile, L"CScript * CScriptManager::GetScript(const wstring& strScriptName)\n{\n");

	for (UINT i = 0; i < g_vecName.size(); ++i)
	{
		fwprintf_s(pFile, L"\tif (L\"");
		fwprintf_s(pFile, g_vecName[i].c_str());
		fwprintf_s(pFile, L"\" == strScriptName)\n");
	...
	// 세번째 함수
	fwprintf_s(pFile, L"CScript * CScriptManager::GetScript(UINT iScriptType)\n{\n");

	fwprintf_s(pFile, L"\tswitch (iScriptType)\n\t{\n");
	for (UINT i = 0; i < g_vecName.size(); ++i)
	{
		wstring strScriptUpperName = L"";
		for (UINT j = 1; j < g_vecName[i].size(); ++j)
	...
	// 네번째 함수
	fwprintf_s(pFile, L"const wchar_t * CScriptManager::GetScriptName(CScript *pScript)\n{\n");
	fwprintf_s(pFile, L"\tswitch ((SCRIPT_TYPE)pScript->GetScriptType())\n\t{\n");
	for (UINT i = 0; i < g_vecName.size(); ++i)
	{
